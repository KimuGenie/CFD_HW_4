%Alternating Direction Implicit method y-x sweep

function [u, residual] = ADIyx(u, dx, dy, imax, jmax, maxiter, tolerance)
...
while k<=maxiter
    ...
    for j=2:jmax-1
        for i=2:imax-1
            Ci(i-1) = -beta^2*(uprevj(i, j+1)+u(i, j-1));
        end
        Ci(1) = Ci(1)-u(1, j);
